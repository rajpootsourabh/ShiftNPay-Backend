...
exports.createWeeksRange = async (req, res) => {
  try {
    ...
    if (isNaN(numberOfWeeks) || numberOfWeeks < 1 || numberOfWeeks > 52) {
      return res.status(400).json({ message: "Number of weeks must be between 1 and 52" });
    }

    const parsedPayrollDays = parseInt(payrollDays);
    if (![5, 6, 7].includes(parsedPayrollDays)) {
      return res.status(400).json({ message: "Payroll days must be 5, 6, or 7" });
    }
...
exports.getAllWeeks = async (req, res) => {
  try {
    const weeks = await Models.TimesheetWeek.find({vendorId: req.user._id}).sort({ startDate: 1 });
    res.status(200).json(weeks);
  } catch (err) {
    res.status(500).json({ message: "Error fetching weeks", error: err.message });
  }
};
...
exports.getWeekById = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching week", error: err.message });
  }
};
...
exports.updateWeek = async (req, res) => {
  try {
    ...
    if (!week) return res.status(404).json({ message: "Week not found" });
    if (week.locked) return res.status(400).json({ message: "Cannot update a locked week" });

    const updatedWeek = await Models.TimesheetWeek.findByIdAndUpdate(id, req.body, { new: true });
    res.status(200).json(updatedWeek);
  } catch (err) {
    res.status(500).json({ message: "Error updating week", error: err.message });
  }
};
...
exports.lockWeek = async (req, res) => {
  try {
    const { id } = req.params;
    const week = await Models.TimesheetWeek.findByIdAndUpdate(id, { locked: true }, { new: true });
    if (!week) return res.status(404).json({ message: "Week not found" });
...
exports.unlockWeek = async (req, res) => {
  try {
    const { id } = req.params;
    const week = await Models.TimesheetWeek.findByIdAndUpdate(id, { locked: false }, { new: true });
    if (!week) return res.status(404).json({ message: "Week not found" });
    res.status(200).json({ message: "Week unlocked", data: week });
  } catch (err) {
    res.status(500).json({ message: "Error unlocking week", error: err.message });
  }
};
...
exports.deleteWeek = async (req, res) => {
  try {
    ...
    if (!week) return res.status(404).json({ message: "Week not found" });
    if (week.locked) return res.status(400).json({ message: "Cannot delete a locked week" });

    await Models.TimesheetWeek.findByIdAndDelete(id);
    res.status(200).json({ message: "Week deleted successfully" });
  } catch (err) {
    res.status(500).json({ message: "Error deleting week", error: err.message });
  }
};
