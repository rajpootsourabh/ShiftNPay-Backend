...
exports.getAllEmploye = async (req, res) => {
  try {
    ...
    if (!result) {
      return res
        .status(404)
        .json({ msg: "No employee found!", success: false });
    }
...
exports.getEmpByVendroId = async (req, res) => {
  ...
  try {
    ...
    const employees = await Employee.find({ userId: id, delete: false })
      .select("-password")
      .populate("shift");
    ...
    if (!result) {
      return res
        .status(404)
        .json({ msg: "No employee data found!", success: false });
    }
    return res.status(200).json({ msg: "Ok", success: true, result });
  } catch (error) {
  ...
  try {
    ...
    if (result) {
      return res
        ...
        .json({
          msg: `Employee ${
            checkEmp?.name ? checkEmp?.name : checkEmp?.email
    ...
    return res
      .status(400)
      .json({ msg: "Failed to delete employee", success: false });
  } catch (error) {
...
exports.empLogin = async (req, res) => {
  ...
  try {
    ...
    const checkEmp = await Employee.findOne({ email });
    if (!checkEmp) {
      return res
        .status(404)
        .json({ msg: "No employee data found!", success: false });
    ...
    const matchPass = await bcrypt.compare(password, checkEmp.password);
    if (!matchPass) {
      return res
        .status(400)
        .json({ msg: "Email or Password are incorrect!", success: false });
    }

    const token = createToken({
      _id: checkEmp._id,
      email: checkEmp.email,
      name: checkEmp.name,
      address: checkEmp.address,
      role: "employee",
    });

    if (device_token) {
      await Employee.findOneAndUpdate({ email }, { device_token });
    }

    const unReadNotifications =
      await Services.NotificationService.getUnreadNotificationCount(
        checkEmp._id
      );
    checkEmp.unReadNotifications = unReadNotifications;

    await LoginLog.deleteMany({ employeeId: checkEmp._id });
    ...
    console.error("error on empLogin: ", error);
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({
          msg: `${checkEmp?.name} password has been reset successfully.`,
          success: true,
          result,
        });
    }
    return res
      .status(400)
      .json({
        msg: "Failed to reset your password! Please try again latter",
        success: false,
      });
  } catch (error) {
...
exports.updateEmpyFromVender = async (req, res) => {
  ...
  try {
    ...
    if (image) {
      const fileName =
        "empProfile" + date.getTime() + image.name.replace(/\s+/g, "");
      ...
      image.mv(profileFilePath, (err) => {
        if (err) {
          return res
            ...
            .json({
              success: false,
              msg: "Failed to upload user profile image?",
            });
        }
      });
      ...
      if (checkEmp.profile) {
        const removeprofileFilePath = path.join(
          __dirname,
          "..",
          "assets",
          "empProfile",
        ...
        fs.unlink(removeprofileFilePath, (err) => {
          if (err) {
            return res
              .status(400)
              .json({ msg: "Failed to delete profile image?", success: false });
          }
        });
      }
    }
    ...
    if (result) {
      return res
        .status(200)
        .json({
          msg: `Employee ${name} data updated successfully.`,
          success: true,
          result,
        });
    }
    return res
      .status(400)
      .json({ msg: "Failed to update employee data!", success: false });
  } catch (error) {
...
exports.updateAbount = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ msg: `Profile updated successfully.`, success: true });
    }
    return res
      .status(400)
      .json({ msg: `Failed to update profile!`, success: false });
  } catch (error) {
...
exports.deleteJobFromEmp = async (req, res) => {
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ msg: "Job removed successfully.", success: true, result });
    }
    return res
      .status(400)
      .json({ msg: "Failed to delete job!", success: false });
  } catch (error) {
...
exports.getProfile = async (req, res) => {
  ...
  try {
    ...
    const result = await Shift.find({ userId: employee._id })
...
exports.updateProfile = async (req, res) => {
  ...
  if (formData.shiftPreferences)
    updates.shiftPreferences = formData.shiftPreferences;
  ...
  if (formData.availabilityPreference)
    updates.availabilityPreference = formData.availabilityPreference;
  ...
  try {
    ...
    if (result) {
      return res
        .status(200)
        .json({ msg: "Employee data updated successfully.", success: true });
    }
    return res
      .status(400)
      .json({ msg: "Failed to update employee data!", success: false });
...
exports.updateProfileEmp = async (req, res) => {
  ...
  try {
    ...
    if (!profile) {
      return res
        .status(400)
        .json({ msg: `Profile image is required!`, success: false });
    }

    const fileName =
      "empProfile" + date.getTime() + profile.name.replace(/\s+/g, "");
    const profileFilePath = path.join(
      __dirname,
      "..",
      "assets",
    ...
    return res
      .status(400)
      .json({ msg: "Failed to update profile image", success: false });
  } catch (error) {
...
exports.notificationsList = async (req, res) => {
  try {
    ...
    const result = await Services.NotificationService.getNotificationsByUser(
      checkEmp._id
    );
    if (result) {
...
exports.notificationsUnreadCount = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: `Success`, success: true, unReadNotifications });
  } catch (error) {
    //console.log("error on updateProfileEmp: ", error);
    return res
      .status(500)
...
exports.notificationsMarkAllAsRead = async (req, res) => {
  try {
    ...
    const result = await Services.NotificationService.markAllAsRead(
      checkEmp._id
    );
    if (result) {
...
exports.notificationsTest = async (req, res) => {
  try {
    ...
    const result = await Services.NotificationService.sendNotification(
      checkUser._id,
      checkEmp.device_token,
      "Test Notification",
      "Test Description",
      {}
    );
...
exports.testNotificationCreate = async (req, res) => {
  try {
    ...
    const result = await Services.NotificationService.sendNotification(
      ...
      checkEmp.device_token,
      "Test Notification",
      "This is a test notification description. this is used for testing purpose only.",
      {}
    );
...
exports.closeAssignedJob = async (req, res) => {
  ...
  try {
    ...
    await Services.NotificationService.sendNotification(
      ...
      vendor?.device_token,
      "Job is Completed!",
      ` ${employee.name} has marked the Job ${job?.name} as Completed from his Side. Please Check the Job for further Actions.`
    );

    return res
      .status(200)
      .json({ msg: "Job closed successfully!", success: true, job });
  } catch (error) {
    console.error("Error in closeJob: ", error);
    return res
      .status(500)
      .json({ msg: "Server Error", err: error.message, success: false });
  }
};
